...
class LibraryPreview(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.app.version[0] + (bpy.app.version[1] / 100.0) < 2.66 and category_special != "OSL Scripts" and category_special != "Nodegroups":
            ...
            output_node = bpy.data.materials["mat_lib_preview_material"].node_tree.nodes.new("OUTPUT_MATERIAL")
            output_node.location = [200, 100]
            emission_node = bpy.data.materials["mat_lib_preview_material"].node_tree.nodes.new("EMISSION")
            emission_node.location = [0, 100]
            emission_node.inputs['Strength'].default_value = 1.0
            image_node = bpy.data.materials["mat_lib_preview_material"].node_tree.nodes.new("TEX_IMAGE")
            image_node.location = [-200, 100]
            image_node.image = bpy.data.images['mat_lib_preview_image.jpg']
            if material_tileabilities[current_material_number] == "yes":
                mapping_node = bpy.data.materials["mat_lib_preview_material"].node_tree.nodes.new("MAPPING")
                mapping_node.location = [-500, 100]
                mapping_node.scale = [2.0, 2.0, 2.0]
                coords_node = bpy.data.materials["mat_lib_preview_material"].node_tree.nodes.new("TEX_COORD")
...
class InsertLibraryImage(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        new_node = context.active_object.active_material.node_tree.nodes.new('TEX_IMAGE')
...
def addNodes(nodes, node_tree, group_mode = False):
    ...
    for dom_node in nodes:
        ...
        if node_type == "ATTRIBUTE":
            print ("ATTRIBUTE")
            node = node_tree.nodes.new(blender267[node_type])
